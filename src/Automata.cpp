...
Automata::Automata(std::vector <std::string> Menu, std::vector <int> Prices) : Automata() {
	if(Menu.size()==Prices.size()){
		menu = Menu;
		prices = Prices;
	} else {
		std::cout << "Incorrect data! The basic menu has been installed." << std::endl;
		Automata();
	}
}
...
void Automata::off() {
	if (state == WAIT) {
		...
		std::cout << "The coffe machine can't be turned off at the moment." << std::endl;
	}
}
...
void Automata::choice(int num_of_choice) {
	if (state == ACCEPT) {
		if (num_of_choice > menu.size()||num_of_choice<=0) {
			...
			std::cout << "You have chosen " << menu[num_of_choice-1] << " for " << prices[num_of_choice - 1] << " rub." << std::endl;
			check();
		}
	} else {
		std::cout << "The opperation can't be done." << std::endl;
...
void Automata::check() {
	if (state == CHECK) {
		if (prices[num_of_chousen_drink] <= cash) {
			std::cout << "You have deposited enough money, the drink can be cooked." << std::endl;
		} else {
...
void Automata::cancel() {
	if (state == ACCEPT || state == CHECK) {
		...
		if (cash > 0) std::cout << " Please, take your cash: "<< cash<<" rub." << std::endl;
		cash = 0;
		num_of_chousen_drink = -1;
		state = WAIT;
	} else {
...
void Automata::finish() {
	if (state == COOK) {
		state = WAIT;
		if (cash > 0) std::cout << "Please, take the change: "<<cash<<" rub." << std::endl;
		cash = 0;
		num_of_chousen_drink = -1;
